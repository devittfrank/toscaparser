/** 
 * Parsing Tosca report (path/to/report/*.xml)
 */
...
async function buildTestResultByClassName(obj) {
  ...
  let buildAttachmentAndTestStepLogs = async (testcases) => {
    ...
    for (let tc of testcases) {
      if (tc) {
        testStepLogs.push({
          order: order++,
          status: (undefined != tc.skipped) ? "SKIP" : tc.failure ? "FAIL" : "PASS",
          description: tc.$.name,
  ...
  let status = (+obj.$.tests !== 0) && (+obj.$.failures === 0) ? "PASS" : "FAIL";
...
async function parse(pathToTestResult, options) {
  ...
  for (let file of resultFiles) {
    ...
    if (true === useClassNameAsTestCaseName) {
      ...
      let tcObj = await buildTestResultByClassName(testSuite);
